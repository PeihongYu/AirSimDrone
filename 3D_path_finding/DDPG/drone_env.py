...
class drone_env_gridworld(drone_env):
    ...
    def step(self, action):
        ...
        pos_ = v2t(self.client.getMultirotorState().kinematics_estimated.position)
        vel_ = v2t(self.client.getMultirotorState().kinematics_estimated.linear_velocity)
    ...
    def reset(self):
        ...
        pos = v2t(self.client.getMultirotorState().kinematics_estimated.position)
        vel = v2t(self.client.getMultirotorState().kinematics_estimated.linear_velocity)
...
class drone_env_heightcontrol(drone_env):
    ...
    def reset(self):
        if self.rand:
            self.reset_aim()
        drone_env.reset(self)
        self.state, self.pose = self.getState()
        return self.state
    ...
    def step(self, action):
        # pos = v2t(self.client.getPosition())
        # cur_pos = v2t(self.client.getMultirotorState().kinematics_estimated.position)
        state_, cur_pos = self.getState()
        dpos = self.aim - cur_pos
        ...
        reward = self.rewardf(self.state, state_)
        # reward = self.rewardf(self.pose, cur_pos)
        ...
        if pos < self.height_limit:
            done = True
            info = "too high"
            reward = -50
        if distance(cur_pos, self.pose) < 1e-3:
            done = True
            info = "freeze"
            reward = -50
        ...
        return norm_state, reward, done, info, cur_pos
